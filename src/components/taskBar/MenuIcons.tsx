...
function MenuIcons(props: { icon: string; title: string, windowName: string, isOpen: boolean, position: Position }) {
  const { setIsOpen, setShowMenu } = useOpenHooks();  

  function test(){
    changeWindowState(props.windowName, props.isOpen, props.position, setIsOpen)    
    setShowMenu(false);    
  }

  return (
    <main className="w-1/4 flex flex-col justify-center items-center">
      <button className="text-white text-4xl" onClick={() => test()}>
